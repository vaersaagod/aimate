...
class AssetController extends Controller
{
    ...
    public function actionAltText(): int
    {
        $config = AIMate::getInstance()->getSettings();
        $query = $this->_getAssetQuery();
    ...
    public function actionFocalPoint(): int
    {
        $config = AIMate::getInstance()->getSettings();
        $query = $this->_getAssetQuery();
        ...
        foreach ($assets as $asset) {
            $this->stdout("Requesting focal point for asset “{$asset->filename}” ({$asset->id})... ", BaseConsole::FG_CYAN);
    ...
    private function _getAssetQuery()
    {
        $query = Asset::find();
